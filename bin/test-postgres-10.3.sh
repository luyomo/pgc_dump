...
ssh_user=postgres
ssh_pwd=postgres
pg_path=/usr/local/postgres10.3/bin
...
psql -h $source_host -p $source_port -U $source_user -d $source_db -v db="'$source_db'" -v ver="'$source_version'" -f /test/cp_from/cp_from_10.3.sh >/dev/null 2>&1
...
psql -h $pgc_host -p $pgc_port -U $pgc_user -d $pgc_db -v db="'$source_db'" -v ver="'$source_version'" -f /test/cp_to/cp_to_10.3.sh >/dev/null 2>&1
