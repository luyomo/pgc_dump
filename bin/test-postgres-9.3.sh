...
rm -rf /tmp/*.csv /tmp/copy_sequence.sql /tmp/source_table.dmp /tmp/pgc_dump.dmp
...
psql -h $source_host -p $source_port -U $source_user -d $source_db -v db="'$source_db'" -v ver="'$source_version'" -f /test/cp_from/cp_from_9.3.sh >/dev/null 2>&1
...
psql -h $pgc_host -p $pgc_port -U $pgc_user -d $pgc_db -v db="'$source_db'" -v ver="'$source_version'" -f /test/cp_to/cp_to_9.3.sh >/dev/null 2>&1
...
#clean test table
export PGPASSWORD=$source_pwd
psql -h $source_host -p $source_port -U $source_user -d $source_db -f /test/cp_from/clean-ddl.sql
