...
#define fmtQualifiedDumpable(obj) \
	fmtQualifiedId((obj)->dobj.schename, \
				   (obj)->dobj.name)
...
static void expand_table_name_patterns(Archive *fout,
						   ...
						   SimpleOidList *oids,
						   bool strict_names);
static void expand_table_name_patterns_db_ver(Archive *fout,
						   SimpleStringList *patterns,
						   SimpleOidList *oids,
						   const char *db,const char *ver,
						   bool strict_names);
static void expand_func_name_patterns_db_ver(Archive *fout,
						   SimpleStringList *patterns,
						   SimpleOidList *oids,
						   const char *db,const char *ver,
						   bool strict_names);
...
/* add for function export */
static SimpleStringList func_include_patterns = {NULL, NULL};
static SimpleOidList func_include_oids = {NULL, NULL};
...
main(int argc, char **argv)
{
	...
	static struct option long_options[] = {
		...
		{"dbnameE", required_argument, NULL, 'D'},
	...
	set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("pg_catalog"));
...
//		}
	...
	while ((c = getopt_long(argc, argv, "abBcCd:E:f:F:h:j:n:N:oOp:RsS:t:T:U:vwWxZ:D:V:P:",
							long_options, &optindex)) != -1)
	{
		switch (c)
		{
			case 'D':  /* db_name  */
				dopt.db = pg_strdup(optarg);
				//printf("get db option here == %s \n",dopt.db);
				break;

			case 'V': /* export function  */
				dopt.ver = pg_strdup(optarg);
				//printf("get ver option here == %s \n",dopt.ver);
				break;

			case 'P':			/* export function  */
				//dopt.funcs = pg_strdup(optarg);
				simple_string_list_append(&func_include_patterns, optarg);
				break;
	...
	if (fout->remoteVersion >= 80100)
		username_subquery = "SELECT rolname FROM pgc_dump.pg_roles WHERE oid =";
	else
		username_subquery = "SELECT usename FROM pgc_dump.pg_user WHERE usesysid =";
	...
	{
		expand_table_name_patterns_db_ver(fout, &table_include_patterns,
								   &table_include_oids,
								   dopt.db,dopt.ver,
								   strict_names);
	...
	if (func_include_patterns.head != NULL)
	{
		expand_func_name_patterns_db_ver(fout, &func_include_patterns,
								   &func_include_oids,
								   dopt.db,dopt.ver,
								   strict_names);
		if (func_include_oids.head == NULL)
			exit_horribly(NULL, "no matching function were found\n");
	}
	/* non-matching exclusion patterns aren't an error */
...
//	printf("-------------  4 -------------\n");
	...
	for (i = 0; i < numObjs; i++){
//		printf("dobjs[i]->name =======%d %s \n",i,dobjs[i]->name);
//		printf("dobjs[i]->objType =======%d %d \n",i,dobjs[i]->objType);
...
{
	char	   *query = "SELECT pgc_dump.pg_export_snapshot()";
	char	   *result;
	PGresult   *res;
							...
							bool strict_names)
{
	...
	for (cell = patterns->head; cell; cell = cell->next)
	{
		appendPQExpBuffer(query,
						  "SELECT oid FROM pgc_dump.pg_namespace n\n");
						   ...
						   bool strict_names)
{
	...
	for (cell = patterns->head; cell; cell = cell->next)
	{
		...
		appendPQExpBuffer(query,
						  "SELECT c.oid"
						  "\nFROM pgc_dump.pg_class c"
						  "\n     LEFT JOIN pgc_dump.pg_namespace n"
						  "\n     ON n.oid = c.relnamespace"
						  "\nWHERE c.relkind OPERATOR(pgc_dump.=) ANY"
						  "\n    (array['%c', '%c', '%c', '%c', '%c', '%c'])\n",
		...
		res = ExecuteSqlQuery(fout, query->data, PGRES_TUPLES_OK);
		printf(">> expand_table_name_patterns  query = %s \n",query->data);
...
static void
expand_table_name_patterns_db_ver(Archive *fout,
						   SimpleStringList *patterns, SimpleOidList *oids,
						   const char *db,const char *ver,
						   bool strict_names)
{
	...
	for (cell = patterns->head; cell; cell = cell->next)
	{
		...
		appendPQExpBuffer(query,
						  ...
						  "\nFROM pgc_dump.pg_class c"
						  "\n     LEFT JOIN pgc_dump.pg_namespace n"
						  "\n     ON n.oid = c.relnamespace AND c.db_name = n.db_name and c.db_version = n.db_version"
						  "\nWHERE c.relkind OPERATOR(pgc_dump.=) ANY"
						  "\n    (array['%c', '%c', '%c', '%c', '%c', '%c'])"
				          "\n    and c.db_name = '%s' and c.db_version = '%s'",
						  RELKIND_RELATION, RELKIND_SEQUENCE, RELKIND_VIEW,
						  RELKIND_MATVIEW, RELKIND_FOREIGN_TABLE,
						  RELKIND_PARTITIONED_TABLE,db,ver
						  );
		processSQLNamePattern(GetConnection(fout), query, cell->val, true,
							  false, "n.nspname", "c.relname", NULL,
							  "1=1");
...
static void
expand_func_name_patterns_db_ver(Archive *fout,
						   SimpleStringList *patterns, SimpleOidList *oids,
						   const char *db,const char *ver,
						   bool strict_names)
{
	...
	for (cell = patterns->head; cell; cell = cell->next)
	{
		...
		appendPQExpBuffer(query,
						  "SELECT c.oid"
						  "\nFROM pgc_dump.pg_proc c"
						  "\n     LEFT JOIN pgc_dump.pg_namespace n"
						  "\n     ON n.oid = c.pronamespace AND c.db_name = n.db_name and c.db_version = n.db_version"
						  "\n WHERE c.db_name = '%s' and c.db_version = '%s'",
						  db,ver
						  );
		processSQLNamePattern(GetConnection(fout), query, cell->val, true,
							  false, "n.nspname", "c.proname", NULL,
							  "1=1");
		//printf(" db  =================== = %s \n",db);
		ExecuteSqlStatement(fout, "RESET search_path");
//		printf("vexpand_table_name_patterns query = %s \n",query->data);
		res = ExecuteSqlQuery(fout, query->data, PGRES_TUPLES_OK);
		PQclear(ExecuteSqlQueryForSingleRow(fout,
											ALWAYS_SECURE_SEARCH_PATH_SQL));
		if (strict_names && PQntuples(res) == 0)
	...
	if (table_include_oids.head != NULL)
		...
		nsinfo->dobj.dump_contains = nsinfo->dobj.dump =
			...
			DUMP_COMPONENT_ALL : DUMP_COMPONENT_NONE;
			 ...
			 strcmp(nsinfo->dobj.name, "pgc_dump") == 0)
	{
		/*
		 * In 9.6 and above, we dump out any ACLs defined in pgc_dump, if
...
selectDumpableProcLang(ProcLangInfo *plang, Archive *fout)
{
	 ...
	 * the pgc_dump namespace.  We need this because procedural languages do
...
dumpDatabase(Archive *fout)
{
	...
	if (strcmp(datistemplate, "t") == 0)
	{
		 ...
		 */
		appendPQExpBufferStr(delQry, "UPDATE pgc_dump.pg_database "
	...
	if (dopt->binary_upgrade)
	{
		...
		appendPQExpBuffer(creaQry, "UPDATE pgc_dump.pg_database\n"
	...
	if (dopt->binary_upgrade)
	{
		...
		appendPQExpBuffer(loOutQry, "UPDATE pgc_dump.pg_class\n"
...
dumpSearchPath(Archive *AH)
{
	...
	res = ExecuteSqlQueryForSingleRow(AH,
									  "SELECT pgc_dump.current_schemas(false)");
	...
	appendPQExpBufferStr(qry, "SELECT pgc_dump.set_config('search_path', ");
...
dumpBlob(Archive *fout, BlobInfo *binfo)
{
	...
	appendPQExpBuffer(cquery,
					  "SELECT pgc_dump.lo_create('%s');\n",
					  binfo->dobj.name);

	appendPQExpBuffer(dquery,
					  "SELECT pgc_dump.lo_unlink('%s');\n",
					  binfo->dobj.name);
	...
	for (i = 0; i < numTables; i++)
	{
		...
		if (fout->remoteVersion >= 100000)
			appendPQExpBuffer(query,
							  ...
							  "   array_to_string(ARRAY(SELECT pgc_dump.quote_ident(rolname) from pgc_dump.pg_roles WHERE oid = ANY(pol.polroles)), ', ') END AS polroles, "
							  "pgc_dump.pg_get_expr(pol.polqual, pol.polrelid) AS polqual, "
							  "pgc_dump.pg_get_expr(pol.polwithcheck, pol.polrelid) AS polwithcheck "
							  "FROM pgc_dump.pg_policy pol "
							  "WHERE polrelid = '%u'",
							  tbinfo->dobj.catId.oid);
		else
			appendPQExpBuffer(query,
...
getPublications(Archive *fout)
{
	...
	if (fout->remoteVersion >= 110000)
		appendPQExpBuffer(query,
						  ...
						  "(%s p.pubowner and db_name = p.db_name and db_version = p.db_version) AS rolname, "
						  "p.puballtables, p.pubinsert, p.pubupdate, p.pubdelete, p.pubtruncate "
						  "FROM pgc_dump.pg_publication p",
						  username_subquery);
	else
		appendPQExpBuffer(query,
						  "SELECT p.tableoid, p.oid, p.pubname, "
...
getPublicationTables(Archive *fout, TableInfo tblinfo[], int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
		...
		appendPQExpBuffer(query,
						  ...
						  "FROM pgc_dump.pg_publication_rel pr, pgc_dump.pg_publication p "
						  "WHERE pr.prrelid = '%u'"
...
getSubscriptions(Archive *fout)
{
	...
	if (!is_superuser(fout))
	{
		...
		res = ExecuteSqlQuery(fout,
							  "SELECT count(*) FROM pgc_dump.pg_subscription ps "
							  "WHERE subdbid = (SELECT oid FROM pgc_dump.pg_database"
							  "                 WHERE datname = current_database() and db_name = ps.db_name and db_version = ps.db_version)",
							  PGRES_TUPLES_OK);
	...
	appendPQExpBuffer(query,
					  ...
					  "(%s s.subowner  and db_name = s.db_name and db_version = s.db_version) AS rolname, "
					  " s.subconninfo, s.subslotname, s.subsynccommit, "
					  " s.subpublications "
					  "FROM pgc_dump.pg_subscription s "
					  "WHERE s.subdbid = (SELECT oid FROM pgc_dump.pg_database pd "
					  "                   WHERE datname = current_database() and db_name = s.db_name and db_version = s.db_version)",
					  username_subquery);
	res = ExecuteSqlQuery(fout, query->data, PGRES_TUPLES_OK);
										 ...
										 bool force_array_type)
{
	...
	appendPQExpBuffer(upgrade_buffer,
					  "SELECT pgc_dump.binary_upgrade_set_next_pg_type_oid('%u'::pg_catalog.oid);\n\n",
					  pg_type_oid);

	/* we only support old >= 8.3 for binary upgrades */
	appendPQExpBuffer(upgrade_query,
					  "SELECT typarray "
					  "FROM pgc_dump.pg_type "
	...
	if (OidIsValid(pg_type_array_oid))
	{
		...
		appendPQExpBuffer(upgrade_buffer,
						  "SELECT pgc_dump.binary_upgrade_set_next_array_pg_type_oid('%u'::pg_catalog.oid);\n\n",
						  pg_type_array_oid);
	}
										...
										Oid pg_rel_oid)
{
	...
	appendPQExpBuffer(upgrade_query,
					  ...
					  "FROM pgc_dump.pg_class c "
					  "LEFT JOIN pgc_dump.pg_class t ON "
	...
	if (!PQgetisnull(upgrade_res, 0, PQfnumber(upgrade_res, "trel")))
	{
		...
		appendPQExpBuffer(upgrade_buffer,
						  "SELECT pgc_dump.binary_upgrade_set_next_toast_pg_type_oid('%u'::pg_catalog.oid);\n\n",
						  pg_type_toast_oid);
								 ...
								 bool is_index)
{
	...
	appendPQExpBuffer(upgrade_query,
					  ...
					  "FROM pgc_dump.pg_class c LEFT JOIN "
					  "pgc_dump.pg_index i ON (c.reltoastrelid = i.indrelid AND i.indisvalid) "
					  "WHERE c.oid = '%u'::pg_catalog.oid;",
					  pg_class_oid);
	...
	{
		appendPQExpBuffer(upgrade_buffer,
						  "SELECT pgc_dump.binary_upgrade_set_next_heap_pg_class_oid('%u'::pg_catalog.oid);\n",
						  pg_class_oid);
		...
		if (OidIsValid(pg_class_reltoastrelid))
		{
			...
			appendPQExpBuffer(upgrade_buffer,
							  "SELECT pgc_dump.binary_upgrade_set_next_toast_pg_class_oid('%u'::pg_catalog.oid);\n",
							  pg_class_reltoastrelid);

			/* every toast table has an index */
			appendPQExpBuffer(upgrade_buffer,
							  "SELECT pgc_dump.binary_upgrade_set_next_index_pg_class_oid('%u'::pg_catalog.oid);\n",
							  pg_index_indexrelid);
		}
	}
	else
...
getNamespaces(Archive *fout, int *numNamespaces)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT n.tableoid, n.oid, n.nspname, "
						  "(%s nspowner and db_name = n.db_name and db_version = n.db_version) AS rolname, "
						  "%s as nspacl, "
						  "%s as rnspacl, "
						  "%s as initnspacl, "
						  "%s as initrnspacl "
						  "FROM pgc_dump.pg_namespace n "
						  "LEFT JOIN pgc_dump.pg_init_privs pip "
						  "ON (n.oid = pip.objoid "
						  "AND pip.classoid = 'pg_namespace'::regclass "
						  "AND pip.objsubid = 0 and n.db_name = pip.db_name and n.db_version = pip.db_version ",
						  username_subquery,
						  acl_subquery->data,
						  racl_subquery->data,
						  init_acl_subquery->data,
		...
		appendPQExpBuffer(query, ") "
						"where n.db_name = '%s' and n.db_version = '%s' ",
						 dopt->db,dopt->ver);
...
getTypes(Archive *fout, int *numTypes)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT t.toid as tableoid, t.oid, t.typname, "
						  "t.typnamespace, "
						  "%s AS typacl, "
						  "%s AS rtypacl, "
						  "%s AS inittypacl, "
						  "%s AS initrtypacl, "
						  "(%s t.typowner and db_name = t.db_name and db_version = t.db_version) AS rolname, "
						  "t.typelem, t.typrelid, "
						  "CASE WHEN t.typrelid = 0 THEN ' '::\"char\" "
						  "ELSE (SELECT relkind FROM pgc_dump.pg_class WHERE oid = t.typrelid and db_name = t.db_name and db_version = t.db_version) END AS typrelkind, "
						  "t.typtype, t.typisdefined, "
						  "t.typname[0] = '_' AND t.typelem != 0 AND "
						  "(SELECT typarray FROM pgc_dump.pg_type te WHERE oid = t.typelem and db_name = t.db_name and db_version = t.db_version) = t.oid AS isarray "
						  "FROM pgc_dump.pg_type t "
						  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(t.oid = pip.objoid "
						  "AND pip.classoid = 'pg_type'::regclass "
						  "AND pip.objsubid = 0 and pip.db_name = t.db_name and pip.db_version = t.db_version) "
						  "where t.db_name = '%s' and t.db_version = '%s'",
						  acl_subquery->data,
						  racl_subquery->data,
						  initacl_subquery->data,
						  initracl_subquery->data,
						  username_subquery,dopt->db,dopt->ver);
	...
	else if (fout->remoteVersion >= 90200)
	{
		appendPQExpBuffer(query, "SELECT tableoid, oid, typname, "
						  ...
						  "(%s typowner and db_name = t.db_name and db_version = t.db_version) AS rolname, "
						  "typelem, typrelid, "
						  "CASE WHEN typrelid = 0 THEN ' '::\"char\" "
						  "ELSE (SELECT relkind FROM pgc_dump.pg_class WHERE oid = typrelid and db_name = t.db_name and db_version = t.db_version) END AS typrelkind, "
						  "typtype, typisdefined, "
						  "typname[0] = '_' AND typelem != 0 AND "
						  "(SELECT typarray FROM pgc_dump.pg_type te WHERE oid = pg_type.typelem and db_name = t.db_name and db_version = t.db_version) = oid AS isarray "
						  "FROM pgc_dump.pg_type t",
						  username_subquery);
	}
...
getOperators(Archive *fout, int *numOprs)
{
	...
	appendPQExpBuffer(query, "SELECT tableoid, oid, oprname, "
					  "oprnamespace, "
					  "coalesce((%s oprowner),'pgc_dump') AS rolname, "
					  "oprkind, "
					  "oprcode::oid AS oprcode "
					  "FROM pgc_dump.pg_operator",
					  username_subquery);
...
//	}
	...
	appendPQExpBuffer(query, "SELECT tableoid, oid, collname, "
					  "collnamespace, "
					  "(%s collowner and db_name = pc.db_name and db_version = pc.db_version) AS rolname "
					  "FROM pgc_dump.pg_collation pc",
					  username_subquery);
...
getAggregates(Archive *fout, int *numAggs)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT p.tableoid, p.oid, "
						  ...
						  "(%s p.proowner and db_name = p.db_name and db_version = p.db_version) AS rolname, "
						  "%s AS aggacl, "
						  "%s AS raggacl, "
						  "%s AS initaggacl, "
						  "%s AS initraggacl "
						  "FROM pgc_dump.pg_proc p "
						  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(p.oid = pip.objoid "
						  "AND pip.classoid = 'pg_proc'::regclass "
						  "AND pip.objsubid = 0 and pip.db_name = p.db_name and pip.db_version = p.db_version) "
						  "WHERE %s AND ("
						  "p.pronamespace != "
						  "(SELECT oid FROM pgc_dump.pg_namespace "
						  "WHERE nspname = 'pgc_dump' and db_name = p.db_name and db_version = p.db_version) OR "
		...
		if (dopt->binary_upgrade)
			appendPQExpBufferStr(query,
								 " OR EXISTS(SELECT 1 FROM pgc_dump.pg_depend WHERE "
	...
	else
	{
		appendPQExpBuffer(query, "SELECT tableoid, oid, proname AS aggname, "
						  "pronamespace AS aggnamespace, "
						  "CASE WHEN proargtypes[0] = 'pgc_dump.\"any\"'::regtype THEN 0 ELSE 1 END AS pronargs, "
						  "proargtypes, "
						  "(%s proowner) AS rolname, "
						  "proacl AS aggacl, "
						  "NULL AS raggacl, "
						  "NULL AS initaggacl, NULL AS initraggacl "
						  "FROM pgc_dump.pg_proc "
						  "WHERE proisagg "
						  "AND pronamespace != "
						  "(SELECT oid FROM pgc_dump.pg_namespace WHERE nspname = 'pgc_dump')",
						  username_subquery);
	}
...
static void
selectDumpableFunction(FuncInfo *tbinfo, Archive *fout)
{
	if (checkExtensionMembership(&tbinfo->dobj, fout))
		return;					/* extension membership overrides all else */
	 ...
	 * according to the parent namespace's dump flag.
	 */
//	printf("func_include_oids.head  === %s \n",func_include_oids.head);
	if (func_include_oids.head != NULL){
		tbinfo->dobj.dump = simple_oid_list_member(&func_include_oids,
												   tbinfo->dobj.catId.oid) ?
			DUMP_COMPONENT_ALL : DUMP_COMPONENT_NONE;
		//printf("tbinfo->dobj.dump  === %d \n",tbinfo->dobj.dump);
	}else
		tbinfo->dobj.dump = tbinfo->dobj.namespace->dobj.dump_contains;
}
/*
...
getFuncs(Archive *fout, int *numFuncs)
{
	 ...
	 * ordering correct.
	 *
	 * 3. Otherwise, we normally exclude functions in pgc_dump.  However, if
	 ...
	 * pgc_dump if they have an ACL different from what's shown in
	 * pg_init_privs.
	 */
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query,
						  ...
						  "%s AS initrproacl, "
						  "p.pronamespace, "
				"(select nspname from pgc_dump.pg_namespace ppg where oid = p.pronamespace and db_name = p.db_name and db_version = p.db_version) as schename,"
						  "(%s p.proowner and db_name = p.db_name and db_version = p.db_version) AS rolname "
						  "FROM pgc_dump.pg_proc p "
						  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(p.oid = pip.objoid "
						  "AND pip.classoid = 'pg_proc'::regclass "
						  "AND pip.objsubid = 0 and pip.db_name = p.db_name and pip.db_version = p.db_version ) "
						  "WHERE %s and p.db_name = '%s' and p.db_version = '%s' "
						  "\n  AND NOT EXISTS (SELECT 1 FROM pgc_dump.pg_depend "
						  "WHERE classid = 'pg_proc'::regclass AND "
						  "objid = p.oid AND deptype = 'i' and db_name = p.db_name and db_version = p.db_version)"
						  "\n  AND ("
						  "\n  pronamespace != "
						  "(SELECT oid FROM pgc_dump.pg_namespace "
						  "WHERE nspname = 'pg_catalog' and db_name = p.db_name and db_version = p.db_version)"
						  "\n  OR EXISTS (SELECT 1 FROM pgc_dump.pg_cast pc"
						  "\n  WHERE pc.oid > %u "
						  "\n  AND p.oid = pc.castfunc and pc.db_name = p.db_name and pc.db_version = p.db_version)"
						  "\n  OR EXISTS (SELECT 1 FROM pgc_dump.pg_transform pt"
						  "\n  WHERE pt.oid > %u AND "
						  "\n  (p.oid = pt.trffromsql"
						  "\n  OR p.oid = pt.trftosql) and pt.db_name = p.db_name and pt.db_version = p.db_version)",
						  acl_subquery->data,
						  racl_subquery->data,
						  initacl_subquery->data,
						  initracl_subquery->data,
	...
	else
	{
		appendPQExpBuffer(query,
				...
				"(select nspname from pgc_dump.pg_namespace ppg where oid = p.pronamespace and db_name = p.db_name and db_version = p.db_version) as schename,"
						  "pronamespace, "
						  "(%s proowner and db_name = p.db_name and db_version = p.db_version) AS rolname "
						  "FROM pgc_dump.pg_proc p "
						  "WHERE NOT proisagg",
						  username_subquery);
		...
		appendPQExpBuffer(query,
						  ...
						  "(SELECT oid FROM pgc_dump.pg_namespace "
						  "WHERE nspname = 'pgc_dump')"
						  "\n  OR EXISTS (SELECT 1 FROM pgc_dump.pg_cast"
		...
		if (fout->remoteVersion >= 90500)
			appendPQExpBuffer(query,
							  "\n  OR EXISTS (SELECT 1 FROM pgc_dump.pg_transform"
...
//	printf(" == getFunc query == %s \n",query->data);
	...
	*numFuncs = ntups;
	...
	i_schename = PQfnumber(res, "schename");
	...
	for (i = 0; i < ntups; i++)
	{
		...
		//printf("----getFuncs-----------  2  -------------\n");
		finfo[i].dobj.namespace->dobj.name = pg_strdup(PQgetvalue(res, i, i_schename));
		finfo[i].dobj.schename = pg_strdup(PQgetvalue(res, i, i_schename));
...
getTables(Archive *fout, int *numTables)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		if (fout->remoteVersion >= 100000)
		{
			partkeydef = "pgc_dump.pg_get_partkeydef(c.oid)";
		...
		appendPQExpBuffer(query,
						  "SELECT c.toid as tableoid, c.oid, c.relname, "
						  "%s AS relacl, %s as rrelacl, "
						  "%s AS initrelacl, %s as initrrelacl, "
						  "c.relkind, c.relnamespace, "
						  "(%s c.relowner and c.db_name = db_name and c.db_version = db_version) AS rolname, "
						  ...
						  "CASE WHEN c.reloftype <> 0 THEN c.reloftype::regtype ELSE NULL END AS reloftype, "
						  "d.refobjid AS owning_tab, "
						  "d.refobjsubid AS owning_col, "
						  "(SELECT spcname FROM pgc_dump.pg_tablespace t WHERE t.oid = c.reltablespace and c.db_name = t.db_name and c.db_version = t.db_version) AS reltablespace, "
						  "array_remove(array_remove(c.reloptions,'check_option=local'),'check_option=cascaded') AS reloptions, "
						  "CASE WHEN 'check_option=local' = ANY (c.reloptions) THEN 'LOCAL'::text "
						  "WHEN 'check_option=cascaded' = ANY (c.reloptions) THEN 'CASCADED'::text ELSE NULL END AS checkoption, "
						  "tc.reloptions AS toast_reloptions, "
						  "c.relkind = '%c' AND EXISTS (SELECT 1 FROM pgc_dump.pg_depend WHERE classid = 'pg_class'::regclass AND objid = c.oid AND objsubid = 0 AND refclassid = 'pg_class'::regclass AND deptype = 'i'"
						  " and c.db_name = db_name and db_version = c.db_version) AS is_identity_sequence, "
						  "EXISTS (SELECT 1 FROM pgc_dump.pg_attribute at LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(c.oid = pip.objoid and c.db_name = at.db_name and c.db_version = at.db_version and pip.db_name = at.db_name and pip.db_version = at.db_version "
						  ...
						  "%s AS partkeydef, "
						  "%s AS ispartition, "
						  "(select nspname from pgc_dump.pg_namespace ppg where oid = c.relnamespace and db_name = c.db_name and db_version = c.db_version) as schename, "
						  "%s AS partbound "
						  "FROM pgc_dump.pg_class c "
						  "LEFT JOIN pgc_dump.pg_depend d ON "
						  "(c.relkind = '%c' AND "
						  "d.classid = c.toid AND d.objid = c.oid AND "
						  "d.objsubid = 0 AND "
						  "d.refclassid = c.toid AND d.deptype IN ('a', 'i') and c.db_name = d.db_name and c.db_version = d.db_version) "
						  "LEFT JOIN pgc_dump.pg_class tc ON (c.reltoastrelid = tc.oid and c.db_name = tc.db_name and c.db_version = tc.db_version) "
						  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(c.oid = pip.objoid "
						  "AND pip.classoid = 'pg_class'::regclass "
						  "AND pip.objsubid = 0 and c.db_name = pip.db_name and c.db_version = pip.db_version) "
				...
				"	and c.db_name = '%s' and c.db_version = '%s' "
						  ...
						  RELKIND_MATVIEW, RELKIND_FOREIGN_TABLE,
						  RELKIND_PARTITIONED_TABLE,dopt->db,dopt->ver);
	...
	else if (fout->remoteVersion >= 90500)
	{
		...
		appendPQExpBuffer(query,
				...
				"(select nspname from pgc_dump.pg_namespace ppg where oid = c.relnamespace and db_name = c.db_name and db_version = c.db_version) as schename, "
						  ...
						  "d.classid = c.toid AND d.objid = c.oid AND "
						  "d.objsubid = 0 AND "
						  "d.refclassid = c.toid AND d.deptype = 'a' and c.db_name = d.db_name and c.db_version = d.db_version) "
						  "LEFT JOIN pgc_dump.pg_class tc ON (c.reltoastrelid = tc.oid and c.db_name = tc.db_name and c.db_version = tc.db_version) "
				...
				"	and c.db_name = '%s' and c.db_version = '%s' "
						  ...
						  RELKIND_RELATION, RELKIND_SEQUENCE,
						  RELKIND_VIEW, RELKIND_COMPOSITE_TYPE,
						  RELKIND_MATVIEW, RELKIND_FOREIGN_TABLE,dopt->db,dopt->ver);
	}
	...
	else if (fout->remoteVersion >= 90000)
	{
		...
		appendPQExpBuffer(query,
				...
				"	and c.db_name = '%s' and c.db_version = '%s' "
						  ...
						  RELKIND_SEQUENCE,
						  RELKIND_RELATION, RELKIND_SEQUENCE,
						  RELKIND_VIEW, RELKIND_COMPOSITE_TYPE,dopt->db,dopt->ver);
	}
	else if (fout->remoteVersion >= 80400)
	{
		/*
	...
	else
	{
		 ...
		 */
		appendPQExpBuffer(query,
						  "SELECT c.toid as tableoid, c.oid, relname, "
						  "relacl, NULL as rrelacl, "
						  "NULL AS initrelacl, NULL AS initrrelacl, "
						  "relkind, relnamespace, "
						  "(%s relowner and c.db_name = db_name and c.db_version = db_version) AS rolname, "
				...
				"(select nspname from pgc_dump.pg_namespace ppg where oid = c.relnamespace and db_name = c.db_name and db_version = c.db_version) as schename, "
						  ...
						  "d.classid = c.toid AND d.objid = c.oid AND "
						  "d.objsubid = 0 AND "
						  "d.refclassid = c.toid AND d.deptype = 'i' and c.db_name = d.db_name and c.db_version = d.db_version) "
	...
	//printf("getTables  query>>>>> = %s \n",query->data);
//	printf("getTables  RELKIND_COMPOSITE_TYPE>>>>> = %d \n",RELKIND_COMPOSITE_TYPE);
	...
	*numTables = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		//printf("getTables  ------------  2  ----------- =  \n");
		tblinfo[i].dobj.namespace->dobj.name = pg_strdup(PQgetvalue(res, i, i_schename));
		tblinfo[i].dobj.schename = pg_strdup(PQgetvalue(res, i, i_schename));

		//tblinfo[i].dobj.schename = pg_strdup(PQgetvalue(res, i, i_schename));
		//printf("tblinfo[i].dobj.name                 = %s\n",tblinfo[i].dobj.name);
		//printf("tblinfo[i].dobj.namespace->dobj.name = %s %s\n",tblinfo[i].dobj.name,tblinfo[i].dobj.namespace->dobj.name);
		//printf("--------------------------------------------------\n");
		 ...
		 */
		//printf(" getTables --- name : relkind : kindName === %s : %d : %s \n",tblinfo[i].dobj.name,tblinfo[i].relkind,pg_strdup(PQgetvalue(res, i, i_relkind)));
		 ...
		 * where we are only looking to dump out the ACL (eg: pgc_dump).
		 */
		...
		//printf(" getTables --- name : relkind : kindName === %s : %d  \n",tblinfo[i].dobj.name,tblinfo[i].dobj.dump);
		/*
...
//		printf(" getOwnedSeqs --- name : dump : iden === %s : %d : %d \n",seqinfo->dobj.name,seqinfo->dobj.dump,seqinfo->is_identity_sequence);
...
//		printf(" getOwnedSeqs --- owning_tab->dobj.dump ===  %d \n",owning_tab->dobj.dump);
//		printf(" getOwnedSeqs --- name : dump : iden === %s : %d : %d \n",seqinfo->dobj.name,seqinfo->dobj.dump,seqinfo->is_identity_sequence);
...
getInherits(Archive *fout, int *numInherits)
{
	 ...
	 * relationship.
	 */
	appendPQExpBufferStr(query, "SELECT inhrelid, inhparent FROM pgc_dump.pg_inherits");
...
getIndexes(Archive *fout, TableInfo tblinfo[], int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
		...
		{
			appendPQExpBuffer(query,
							  "SELECT t.toid as tableoid, t.oid, "
							  ...
							  "c.toid AS contableoid, "
							  "c.oid AS conoid, "
							  "c.condef, "
							  "(SELECT spcname FROM pgc_dump.pg_tablespace s WHERE s.oid = t.reltablespace and s.db_name = i.db_name and s.db_version = i.db_version) AS tablespace, "
							  "t.reloptions AS indreloptions "
							  "FROM pgc_dump.pg_index i "
							  "JOIN pgc_dump.pg_class t ON (t.oid = i.indexrelid and t.db_name = i.db_name and t.db_version = i.db_version) "
							  "JOIN pgc_dump.pg_class t2 ON (t2.oid = i.indrelid and t2.db_name = i.db_name and t2.db_version = i.db_version) "
							  "LEFT JOIN pgc_dump.pg_constraint c "
							  "ON (i.indrelid = c.conrelid AND "
							  "i.indexrelid = c.conindid AND "
							  "c.contype IN ('p','u','x') and c.db_name = i.db_name and c.db_version = i.db_version) "
							  "LEFT JOIN pgc_dump.pg_inherits inh "
							  "ON (inh.inhrelid = indexrelid  and inh.db_name = i.db_name and inh.db_version = i.db_version) "
							  "WHERE i.indrelid = '%u'::pg_catalog.oid "
							  "AND (i.indisvalid OR t2.relkind = 'p') "
							  "AND i.indisready "
					"and i.db_name = '%s' and i.db_version = '%s' "
							  "ORDER BY indexname",
							  tbinfo->dobj.catId.oid,dopt->db,dopt->ver);
		}
		else if (fout->remoteVersion >= 90400)
		{
			/*
		...
		else if (fout->remoteVersion >= 80200)
		{
			appendPQExpBuffer(query,
							  ...
							  "FROM pgc_dump.pg_index i "
							  "JOIN pgc_dump.pg_class t ON (t.oid = i.indexrelid and t.db_name = i.db_name and t.db_version = i.db_version) "
							  "LEFT JOIN pgc_dump.pg_depend d "
							  "ON (d.classid = t.toid "
							  "AND d.objid = t.oid "
							  "AND d.deptype = 'i' and d.db_name = i.db_name and d.db_version = i.db_version) "
							  "LEFT JOIN pgc_dump.pg_constraint c "
							  "ON (d.refclassid = c.toid "
							  "AND d.refobjid = c.oid and c.db_name = i.db_name and c.db_version = i.db_version) "
							  "WHERE i.indrelid = '%u'::pg_catalog.oid "
							  "AND i.indisvalid "
					"and i.db_name = '%s' and i.db_version = '%s' "
							  "ORDER BY indexname",
...
getExtendedStatistics(Archive *fout)
{
	...
	appendPQExpBuffer(query, "SELECT tableoid, oid, stxname, "
					  ...
					  "FROM pgc_dump.pg_statistic_ext",
					  username_subquery);
	...
	for (i = 0; i < numTables; i++)
	{
		...
		if (fout->remoteVersion >= 110000)
			appendPQExpBuffer(query,
							  ...
							  "c.condef "
							  "FROM pgc_dump.pg_constraint c "
							  "WHERE conrelid = '%u'::pg_catalog.oid "
							  "AND conparentid = 0 "
							  "AND contype = 'f'"
							"and db_name = '%s' and db_version = '%s' ",
							  tbinfo->dobj.catId.oid,dopt->db,dopt->ver);
		else
			appendPQExpBuffer(query,
							  "SELECT tableoid, oid, conname, confrelid, "
...
getDomainConstraints(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (fout->remoteVersion >= 90100)
		appendPQExpBuffer(query, "SELECT tableoid, oid, conname, "
						  "pgc_dump.pg_get_constraintdef(oid) AS consrc, "
						  "convalidated "
						  "FROM pgc_dump.pg_constraint "
	...
	for (i = 0; i < numTables; i++)
	{
		...
		if (fout->remoteVersion >= 90000)
		{
			...
			appendPQExpBuffer(query,
							  ...
							  "FROM pgc_dump.pg_trigger t "
							  "WHERE tgrelid = '%u'::pg_catalog.oid "
					"and t.db_name = '%s' and t.db_version = '%s' "
		...
		else
		{
			...
			appendPQExpBuffer(query,
					...
					"and t.db_name = '%s' and t.db_version = '%s' "
							  ...
							  " OR NOT EXISTS"
							  "  (SELECT 1 FROM pgc_dump.pg_depend d "
							  "   JOIN pgc_dump.pg_constraint c ON (d.refclassid = c.toid AND d.refobjid = c.oid and c.db_name = d.db_name and c.db_version = d.db_version) "
							  "   WHERE d.classid = t.toid AND d.objid = t.oid AND d.deptype = 'i' AND c.contype = 'f' and t.db_name = d.db_name and t.db_version = d.db_version))",
							  tbinfo->dobj.catId.oid,dopt->db,dopt->ver);
		}
...
getEventTriggers(Archive *fout, int *numEventTriggers)
{
	...
	appendPQExpBuffer(query,
					  ...
					  "FROM pgc_dump.pg_event_trigger e "
					  "ORDER BY e.oid",
					  username_subquery);
...
getProcLangs(Archive *fout, int *numProcLangs)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT l.tableoid, l.oid, "
						  ...
						  "%s AS initlanacl, "
						  "%s AS initrlanacl, "
						  "(%s l.lanowner and db_name = l.db_name and db_version = l.db_version) AS lanowner "
						  "FROM pgc_dump.pg_language l "
						  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(l.oid = pip.objoid "
						  "AND pip.classoid = 'pg_language'::regclass "
						  "AND pip.objsubid = 0 and pip.db_name = l.db_name and pip.db_version = l.db_version) "
						  "WHERE l.lanispl "
						  "ORDER BY l.oid",
						  acl_subquery->data,
						  racl_subquery->data,
...
getCasts(Archive *fout, int *numCasts)
{
	...
	if (fout->remoteVersion >= 80400)
	{
		appendPQExpBufferStr(query, "SELECT tableoid, oid, "
							 ...
							 "castmethod "
							 "FROM pgc_dump.pg_cast ORDER BY 3,4");
	}
	else
	{
		appendPQExpBufferStr(query, "SELECT tableoid, oid, "
	...
	for (i = 0; i < numTables; i++)
	{
		...
		if (fout->remoteVersion >= 110000)
		{
			...
			appendPQExpBuffer(q, "SELECT a.attnum, a.attname, a.atttypmod, "
							  ...
							  "pgc_dump.format_type(t.oid,a.atttypmod) AS atttypname, "
							  "array_to_string(a.attoptions, ', ') AS attoptions, "
							  "CASE WHEN a.attcollation <> t.typcollation "
							  "THEN a.attcollation ELSE 0 END AS attcollation, "
							  "a.attidentity, "
							  "array_to_string(ARRAY("
							  "SELECT pgc_dump.quote_ident(option_name) || "
							  "' ' || pgc_dump.quote_literal(option_value) "
							  "FROM pgc_dump.pg_options_to_table(attfdwoptions) "
							  "ORDER BY option_name"
							  "), E',\n    ') AS attfdwoptions ,"
							  "CASE WHEN a.atthasmissing AND NOT a.attisdropped "
							  "THEN a.attmissingval ELSE null END AS attmissingval "
							  "FROM pgc_dump.pg_attribute a LEFT JOIN pgc_dump.pg_type t "
							  "ON a.atttypid = t.oid and a.db_name = t.db_name and a.db_version = t.db_version "
							  "WHERE a.attrelid = '%u'::pg_catalog.oid "
							  "AND a.attnum > 0::pg_catalog.int2 "
					"and a.db_name = '%s' and a.db_version = '%s' "
							  "ORDER BY a.attnum",
							  tbinfo->dobj.catId.oid,dopt->db,dopt->ver);
		}
		...
		if (hasdefaults)
		{
			...
			printfPQExpBuffer(q, "SELECT toid as tableoid, oid, adnum, "
							  "adsrc "
							  "FROM pgc_dump.pg_attrdef "
			...
			for (j = 0; j < numDefaults; j++)
			{
				 ...
				 */
				//printf("tbinfo->relkind == RELKIND_VIEW == %d : %d \n",tbinfo->relkind,RELKIND_VIEW);
				if (tbinfo->relkind == RELKIND_VIEW)
				{
...
//	}
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT f.tableoid, f.oid, f.fdwname, "
						  ...
						  "FROM pgc_dump.pg_options_to_table(f.fdwoptions) "
						  "ORDER BY option_name"
						  "), E',\n    ') AS fdwoptions "
						  "FROM pgc_dump.pg_foreign_data_wrapper f "
						  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(f.oid = pip.objoid "
	...
	else if (fout->remoteVersion >= 90100)
	{
		appendPQExpBuffer(query, "SELECT tableoid, oid, fdwname, "
						  ...
						  "SELECT quote_ident(option_name) || ' ' || "
						  "quote_literal(option_value) "
						  "FROM pgc_dump.pg_options_to_table(fdwoptions) "
						  "ORDER BY option_name"
						  "), E',\n    ') AS fdwoptions "
						  "FROM pgc_dump.pg_foreign_data_wrapper",
						  username_subquery);
	}
	else
	{
...
getForeignServers(Archive *fout, int *numForeignServers)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT f.tableoid, f.oid, f.srvname, "
						  ...
						  "SELECT quote_ident(option_name) || ' ' || "
						  "quote_literal(option_value) "
						  "FROM pgc_dump.pg_options_to_table(f.srvoptions) "
						  "ORDER BY option_name"
						  "), E',\n    ') AS srvoptions "
						  "FROM pgc_dump.pg_foreign_server f "
						  "LEFT JOIN pgc_dump.pg_init_privs pip "
						  "ON (f.oid = pip.objoid "
	...
	else
	{
		appendPQExpBuffer(query, "SELECT tableoid, oid, srvname, "
						  ...
						  "SELECT quote_ident(option_name) || ' ' || "
						  "quote_literal(option_value) "
						  "FROM pgc_dump.pg_options_to_table(srvoptions) "
						  "ORDER BY option_name"
						  "), E',\n    ') AS srvoptions "
						  "FROM pgc_dump.pg_foreign_server",
						  username_subquery);
	}
...
getDefaultACLs(Archive *fout, int *numDefaultACLs)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT d.oid, d.tableoid, "
						  ...
						  "%s AS initdefaclacl, "
						  "%s AS initrdefaclacl "
						  "FROM pgc_dump.pg_default_acl d "
						  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
						  "(d.oid = pip.objoid "
...
dumpExtension(Archive *fout, ExtensionInfo *extinfo)
{
	...
	else
	{
		...
		appendPQExpBufferStr(q,
							 "SELECT pgc_dump.binary_upgrade_create_empty_extension(");
...
dumpEnumType(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (dopt->binary_upgrade)
	{
		...
		for (i = 0; i < num; i++)
		{
			...
			appendPQExpBuffer(q,
							  "SELECT pgc_dump.binary_upgrade_set_next_pg_enum_oid('%u'::pg_catalog.oid);\n",
							  enum_oid);
...
dumpRangeType(Archive *fout, TypeInfo *tyinfo)
{
	...
	appendPQExpBuffer(query,
					  "SELECT pgc_dump.format_type(rngsubtype, NULL) AS rngsubtype, "
					  "opc.opcname AS opcname, "
					  "(SELECT nspname FROM pgc_dump.pg_namespace nsp "
					  "  WHERE nsp.oid = opc.opcnamespace) AS opcnsp, "
					  "opc.opcdefault, "
					  "CASE WHEN rngcollation = st.typcollation THEN 0 "
					  "     ELSE rngcollation END AS collation, "
					  "rngcanonical, rngsubdiff "
					  "FROM pgc_dump.pg_range r, pgc_dump.pg_type st, "
					  "     pgc_dump.pg_opclass opc "
...
dumpBaseType(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (fout->remoteVersion >= 90100)
	{
		appendPQExpBuffer(query, "SELECT typlen, "
						  ...
						  "pgc_dump.pg_get_expr(typdefaultbin, 0) AS typdefaultbin, typdefault "
						  "FROM pgc_dump.pg_type "
						  "WHERE oid = '%u'::pg_catalog.oid",
						  tyinfo->dobj.catId.oid);
	}
	...
	else if (fout->remoteVersion >= 80300)
	{
		...
		appendPQExpBuffer(query, "SELECT typlen, "
						  ...
						  "typdelim, typbyval, typalign, typstorage, "
						  "false AS typcollatable, "
						  "pgc_dump.pg_get_expr(typdefaultbin, 'pgc_dump.pg_type'::pg_catalog.regclass) AS typdefaultbin, typdefault "
						  "FROM pgc_dump.pg_type "
						  "WHERE oid = '%u'::pg_catalog.oid",
						  tyinfo->dobj.catId.oid);
	}
...
dumpDomain(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (fout->remoteVersion >= 90100)
	{
		...
		appendPQExpBuffer(query, "SELECT t.typnotnull, "
						  "pgc_dump.format_type(t.typbasetype, t.typtypmod) AS typdefn, "
						  "pgc_dump.pg_get_expr(t.typdefaultbin, 'pgc_dump.pg_type'::pg_catalog.regclass) AS typdefaultbin, "
						  "t.typdefault, "
						  "CASE WHEN t.typcollation <> u.typcollation "
						  "THEN t.typcollation ELSE 0 END AS typcollation "
						  "FROM pgc_dump.pg_type t "
						  "LEFT JOIN pgc_dump.pg_type u ON (t.typbasetype = u.oid) "
	...
	else
	{
		appendPQExpBuffer(query, "SELECT typnotnull, "
						  "pgc_dump.format_type(typbasetype, typtypmod) AS typdefn, "
						  "pgc_dump.pg_get_expr(typdefaultbin, 'pgc_dump.pg_type'::pg_catalog.regclass) AS typdefaultbin, "
...
dumpCompositeType(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (fout->remoteVersion >= 90100)
	{
		...
		appendPQExpBuffer(query, "SELECT a.attname, "
						  "pgc_dump.format_type(a.atttypid, a.atttypmod) AS atttypdefn, "
						  "a.attlen, a.attalign, a.attisdropped, "
						  "CASE WHEN a.attcollation <> at.typcollation "
						  "THEN a.attcollation ELSE 0 END AS attcollation "
						  "FROM pgc_dump.pg_type ct "
						  "JOIN pgc_dump.pg_attribute a ON a.attrelid = ct.typrelid "
						  "LEFT JOIN pgc_dump.pg_type at ON at.oid = a.atttypid "
	...
	else
	{
		...
		appendPQExpBuffer(query, "SELECT a.attname, "
						  ...
						  "a.attlen, a.attalign, a.attisdropped, "
						  "0 AS attcollation "
						  "FROM pgc_dump.pg_type ct, pgc_dump.pg_attribute a "
						  "WHERE ct.oid = '%u'::pg_catalog.oid "
	...
	for (i = 0; i < ntups; i++)
	{
		...
		else
		{
			...
			appendPQExpBuffer(dropped, "UPDATE pgc_dump.pg_attribute\n"
							  "SET attlen = %s, "
...
dumpCompositeTypeColComments(Archive *fout, TypeInfo *tyinfo)
{
	...
	appendPQExpBuffer(query,
					  "SELECT c.toid as tableoid, a.attname, a.attnum "
					  "FROM pgc_dump.pg_class c, pgc_dump.pg_attribute a "
...
dumpProcLang(Archive *fout, ProcLangInfo *plang)
{
	 ...
	 * find them --- if the functions are in the pgc_dump schema, as is
...
dumpFunc(Archive *fout, FuncInfo *finfo)
{
	...
	finfo->dobj.namespace->dobj.name = finfo->dobj.schename;
	query = createPQExpBuffer();
	q = createPQExpBuffer();
	delqry = createPQExpBuffer();
	...
	if (fout->remoteVersion >= 110000)
	{
		...
		appendPQExpBuffer(query,
						  ...
						  "proparallel, "
						  "(SELECT lanname FROM pgc_dump.pg_language WHERE oid = prolang and db_name = pc.db_name and db_version = pc.db_version) AS lanname "
						  "FROM pgc_dump.pg_proc pc "
						  "WHERE oid = '%u'::pg_catalog.oid "
				"and db_name = '%s' and db_version = '%s'",
						  finfo->dobj.catId.oid,dopt->db,dopt->ver);
	}
	else if (fout->remoteVersion >= 90600)
	{
		/*
...
//	printf("== dumpFuncs funcsig_tag = %s \n",funcsig_tag);
//	printf("== dumpFuncs q = %s \n",q->data);
...
dumpOpclass(Archive *fout, OpclassInfo *opcinfo)
{
	...
	if (fout->remoteVersion >= 80300)
	{
		appendPQExpBuffer(query, "SELECT opcintype::regtype, "
						  "opckeytype::regtype, "
						  "opcdefault, opcfamily, "
						  "opfname AS opcfamilyname, "
						  "nspname AS opcfamilynsp, "
						  "(SELECT amname FROM pgc_dump.pg_am WHERE oid = opcmethod) AS amname "
						  "FROM pgc_dump.pg_opclass c "
						  "LEFT JOIN pgc_dump.pg_opfamily f ON f.oid = opcfamily "
						  "LEFT JOIN pgc_dump.pg_namespace n ON n.oid = opfnamespace "
						  "WHERE c.oid = '%u'::pg_catalog.oid",
	...
	else
	{
		appendPQExpBuffer(query, "SELECT opcintype::regtype, "
						  ...
						  "(SELECT amname FROM pgc_dump.pg_am WHERE oid = opcamid) AS amname "
						  "FROM pgc_dump.pg_opclass "
						  "WHERE oid = '%u'::pg_catalog.oid",
						  opcinfo->dobj.catId.oid);
	}
	...
	if (fout->remoteVersion >= 90100)
	{
		appendPQExpBuffer(query, "SELECT amopstrategy, false AS amopreqcheck, "
						  ...
						  "FROM pgc_dump.pg_amop ao JOIN pgc_dump.pg_depend ON "
						  "(classid = 'pgc_dump.pg_amop'::pg_catalog.regclass AND objid = ao.oid) "
						  "LEFT JOIN pgc_dump.pg_opfamily f ON f.oid = amopsortfamily "
						  "LEFT JOIN pgc_dump.pg_namespace n ON n.oid = opfnamespace "
						  "WHERE refclassid = 'pgc_dump.pg_opclass'::pg_catalog.regclass "
	...
	else if (fout->remoteVersion >= 80400)
	{
		appendPQExpBuffer(query, "SELECT amopstrategy, false AS amopreqcheck, "
						  ...
						  "FROM pgc_dump.pg_amop ao, pgc_dump.pg_depend "
						  "WHERE refclassid = 'pgc_dump.pg_opclass'::pg_catalog.regclass "
						  "AND refobjid = '%u'::pg_catalog.oid "
						  "AND classid = 'pgc_dump.pg_amop'::pg_catalog.regclass "
						  "AND objid = ao.oid "
						  "ORDER BY amopstrategy",
						  opcinfo->dobj.catId.oid);
	}
	...
	if (fout->remoteVersion >= 80300)
	{
		appendPQExpBuffer(query, "SELECT amprocnum, "
						  ...
						  "FROM pgc_dump.pg_amproc ap, pgc_dump.pg_depend "
						  "WHERE refclassid = 'pgc_dump.pg_opclass'::pg_catalog.regclass "
						  "AND refobjid = '%u'::pg_catalog.oid "
						  "AND classid = 'pgc_dump.pg_amproc'::pg_catalog.regclass "
						  "AND objid = ap.oid "
						  "ORDER BY amprocnum",
						  opcinfo->dobj.catId.oid);
	}
...
dumpOpfamily(Archive *fout, OpfamilyInfo *opfinfo)
{
	...
	if (fout->remoteVersion >= 90100)
	{
		appendPQExpBuffer(query, "SELECT amopstrategy, false AS amopreqcheck, "
						  ...
						  "LEFT JOIN pgc_dump.pg_opfamily f ON f.oid = amopsortfamily "
						  "LEFT JOIN pgc_dump.pg_namespace n ON n.oid = opfnamespace "
						  "WHERE refclassid = 'pgc_dump.pg_opfamily'::pg_catalog.regclass "
						  "AND refobjid = '%u'::pg_catalog.oid "
	...
	appendPQExpBuffer(query, "SELECT "
					  "(SELECT amname FROM pgc_dump.pg_am WHERE oid = opfmethod) AS amname "
					  "FROM pgc_dump.pg_opfamily "
					  "WHERE oid = '%u'::pg_catalog.oid",
					  opfinfo->dobj.catId.oid);
...
dumpCollation(Archive *fout, CollInfo *collinfo)
{
	...
	if (collprovider[0] == 'c')
		...
		/* to allow dumping pgc_dump; not accepted on input */
		appendPQExpBufferStr(q, "default");
	else
		exit_horribly(NULL,
...
dumpConversion(Archive *fout, ConvInfo *convinfo)
{
	...
	appendPQExpBuffer(query, "SELECT "
					  "pgc_dump.pg_encoding_to_char(conforencoding) AS conforencoding, "
					  "pgc_dump.pg_encoding_to_char(contoencoding) AS contoencoding, "
					  "conproc, condefault "
					  "FROM pgc_dump.pg_conversion c "
					  "WHERE c.oid = '%u'::pg_catalog.oid",
...
dumpAgg(Archive *fout, AggInfo *agginfo)
{
	...
	if (fout->remoteVersion >= 110000)
	{
		appendPQExpBuffer(query, "SELECT aggtransfn, "
						  ...
						  "aggminvtransfn, aggmfinalfn, aggmtranstype::regtype, "
						  ...
						  "true AS convertok, "
						  "pgc_dump.pg_get_function_arguments(p.oid) AS funcargs, "
						  "pgc_dump.pg_get_function_identity_arguments(p.oid) AS funciargs, "
						  "p.proparallel "
						  "FROM pgc_dump.pg_aggregate a, pgc_dump.pg_proc p "
	...
	else if (fout->remoteVersion >= 90400)
	{
		appendPQExpBuffer(query, "SELECT aggtransfn, "
						  ...
						  "aggmfinalfn, aggmtranstype::regtype, "
						  "aggfinalextra, aggmfinalextra, "
						  "'0' AS aggfinalmodify, '0' AS aggmfinalmodify, "
						  "aggsortop, "
						  "aggkind, "
...
dumpTSConfig(Archive *fout, TSConfigInfo *cfginfo)
{
	...
	appendPQExpBuffer(query,
					  "SELECT\n"
					  "  ( SELECT alias FROM pgc_dump.ts_token_type('%u'::pg_catalog.oid) AS t\n"
					  "    WHERE t.tokid = m.maptokentype ) AS tokenname,\n"
					  "  m.mapdict::pg_catalog.regdictionary AS dictname\n"
					  "FROM pgc_dump.pg_ts_config_map AS m\n"
					  "WHERE m.mapcfg = '%u'\n"
		...
		const char *initacls, const char *initracls)
{
	...
	if (dopt->aclsSkip)
		return;
	printf("(dopt->dataOnly && strcmp(type, ) != 0)====== %d \n",(dopt->dataOnly && strcmp(type, "LARGE OBJECT") != 0));
	...
	{
		appendPQExpBuffer(sql, "SELECT pgc_dump.binary_upgrade_set_record_init_privs(true);\n");
		...
		appendPQExpBuffer(sql, "SELECT pgc_dump.binary_upgrade_set_record_init_privs(false);\n");
	}
...
dumpTable(Archive *fout, TableInfo *tbinfo)
{
	 ...
	 */
	//printf("dumpTable== name : kind : RELKIND_SEQUENCE :dump == %s :  %d  : %d : %d \n",tbinfo->dobj.name,tbinfo->relkind,RELKIND_SEQUENCE,tbinfo->dobj.dump );
	if (!tbinfo->dobj.dump || dopt->dataOnly)
		return;
	//printf("dumpTable== name : kind : RELKIND_SEQUENCE == %s :  %d  : %d \n",tbinfo->dobj.name,tbinfo->relkind,RELKIND_SEQUENCE);
	if (tbinfo->relkind == RELKIND_SEQUENCE)
		dumpSequence(fout, tbinfo);
	else
		dumpTableSchema(fout, tbinfo);
//	printf("tbinfo->dobj.dump & DUMP_COMPONENT_ACL = %d \n",(tbinfo->dobj.dump & DUMP_COMPONENT_ACL));
//	printf("tbinfo->relkind ====== %d \n",tbinfo->relkind);
//	printf("RELKIND_SEQUENCE====== %d \n",RELKIND_SEQUENCE);
	/* Handle the ACL here */
...
//	printf("===   dumpTable  fout->remoteVersion  ===================  1  =====%d  \n",(fout->remoteVersion >= 80400 && tbinfo->dobj.dump & DUMP_COMPONENT_ACL));
	/*
	...
	if (fout->remoteVersion >= 80400 && tbinfo->dobj.dump & DUMP_COMPONENT_ACL)
	{
		...
		if (fout->remoteVersion >= 90600)
		{
			...
			appendPQExpBuffer(query,
							  ...
							  "%s AS initattacl, "
							  "%s AS initrattacl "
							  "FROM pgc_dump.pg_attribute at "
							  "JOIN pgc_dump.pg_class c ON (at.attrelid = c.oid) "
							  "LEFT JOIN pgc_dump.pg_init_privs pip ON "
							  "(at.attrelid = pip.objoid "
							  "AND pip.classoid = 'pgc_dump.pg_class'::pg_catalog.regclass "
...
createViewAsClause(Archive *fout, TableInfo *tbinfo)
{
	...
	appendPQExpBuffer(query,
					"SELECT viewdef AS viewdef from pgc_dump.pg_view_def "
					"where toid = '%u'::pg_catalog.oid and db_name = '%s' and db_version = '%s'",
					  tbinfo->dobj.catId.oid,fout->dopt->db,fout->dopt->ver);
...
dumpTableSchema(Archive *fout, TableInfo *tbinfo)
{
	...
	//printf("===== tbinfo->dobj.namespace->dobj.name == %s \n",tbinfo->dobj.namespace->dobj.name);
	// reset the schema
	tbinfo->dobj.namespace->dobj.name = tbinfo->dobj.schename;
	qualrelname = pg_strdup(fmtQualifiedDumpable(tbinfo));
	//printf("===== tbinfo->dobj.namespace->dobj.name == %s \n",tbinfo->dobj.namespace->dobj.name);
	if (dopt->binary_upgrade)
		binary_upgrade_set_type_oids_by_rel_oid(fout, q,
												tbinfo->dobj.catId.oid);
	//printf("===== qualrelname == %s \n",qualrelname);
	...
	else
	{
			...
			case RELKIND_FOREIGN_TABLE:
				{
					...
					appendPQExpBuffer(query,
									  ...
									  "SELECT pgc_dump.quote_ident(option_name) || "
									  "' ' || pgc_dump.quote_literal(option_value) "
									  "FROM pgc_dump.pg_options_to_table(ftoptions) "
									  "ORDER BY option_name"
									  "), E',\n    ') AS ftoptions "
									  "FROM pgc_dump.pg_foreign_table ft "
									  "JOIN pgc_dump.pg_foreign_server fs "
		...
		if (dopt->binary_upgrade)
		{
			for (j = 0; j < tbinfo->numatts; j++)
			{
				if (tbinfo->attmissingval[j][0] != '\0')
				{
					appendPQExpBufferStr(q, "\n-- set missing value.\n");
					appendPQExpBufferStr(q,
										 "SELECT pgc_dump.binary_upgrade_set_missing_value(");
			 ...
			 tbinfo->relkind == RELKIND_PARTITIONED_TABLE))
		{
			for (j = 0; j < tbinfo->numatts; j++)
			{
				...
				{
					appendPQExpBufferStr(q, "\n-- For binary upgrade, recreate dropped column.\n");
					appendPQExpBuffer(q, "UPDATE pgc_dump.pg_attribute\n"
									  "SET attlen = %d, "
				...
				else if (!tbinfo->attislocal[j])
				{
					...
					appendPQExpBufferStr(q, "UPDATE pgc_dump.pg_attribute\n"
			...
			for (k = 0; k < tbinfo->ncheck; k++)
			{
				...
				appendPQExpBuffer(q, " ADD CONSTRAINT %s ",
								  fmtId(constr->dobj.name));
				appendPQExpBuffer(q, "%s;\n", constr->condef);
				appendPQExpBufferStr(q, "UPDATE pgc_dump.pg_constraint\n"
			 ...
			 tbinfo->relkind == RELKIND_MATVIEW))
		{
			...
			appendPQExpBuffer(q, "UPDATE pgc_dump.pg_class\n"
							  "SET relfrozenxid = '%u', relminmxid = '%u'\n"
							  "WHERE oid = ",
			...
			tbinfo->relispopulated)
		{
			...
			appendPQExpBufferStr(q, "UPDATE pgc_dump.pg_class\n"
...
//							  qualrelname);
	...
	if (dopt->binary_upgrade)
		binary_upgrade_extension_member(q, &tbinfo->dobj,
										...
										tbinfo->dobj.namespace->dobj.name);
//	printf("=dumpTableSchema>> q = %s \n ",q->data);
//	printf("=dumpTableSchema>> (tbinfo->dobj.dump & DUMP_COMPONENT_DEFINITION) = %d \n ",(tbinfo->dobj.dump & DUMP_COMPONENT_DEFINITION));
	if (tbinfo->dobj.dump & DUMP_COMPONENT_DEFINITION){
//		printf("--dumpTableSchema>> ArchiveEntry===============before==============\n");
...
dumpIndex(Archive *fout, IndxInfo *indxinfo)
{
	...
	PQExpBuffer q = createPQExpBuffer();
	PQExpBuffer delq = createPQExpBuffer();
	char	   *qindxname = pg_strdup(fmtId(indxinfo->dobj.name));
	//printf(">>dumpIndex=========qindxname     ================%s \n",qindxname);

	if (dopt->dataOnly)
		return;

	tbinfo->dobj.namespace->dobj.name = tbinfo->dobj.schename;
	indxinfo->dobj.namespace->dobj.name = indxinfo->dobj.schename;
	//printf(">>dumpIndex==============    is_constraint     ================%d\n",(!is_constraint));
	/*
...
dumpStatisticsExt(Archive *fout, StatsExtInfo *statsextinfo)
{
	...
	appendPQExpBuffer(query, "SELECT "
					  "pgc_dump.pg_get_statisticsobjdef('%u'::pg_catalog.oid)",
...
dumpSequenceData(Archive *fout, TableDataInfo *tdinfo)
{
	...
	resetPQExpBuffer(query);
	appendPQExpBufferStr(query, "SELECT pgc_dump.setval(");
...
dumpRule(Archive *fout, RuleInfo *rinfo)
{
	...
	else
	{
		...
		appendPQExpBuffer(query,
						  "SELECT pgc_dump.pg_get_ruledef('%u'::pg_catalog.oid)",
...
getDependencies(Archive *fout)
{
	...
	appendPQExpBuffer(query, "SELECT "
						 ...
						 "FROM pgc_dump.pg_depend "
						 "WHERE deptype != 'p' AND deptype != 'e' "
						 "and db_name = '%s' and db_version = '%s' "
						 "ORDER BY 1,2",
						 dopt->db,dopt->ver);
//	printf("getDependencies = query == %s \n",query->data);
	res = ExecuteSqlQuery(fout, query->data, PGRES_TUPLES_OK);
...
getFormattedTypeName(Archive *fout, Oid oid, OidOptions opts)
{
	...
	query = createPQExpBuffer();
	appendPQExpBuffer(query, "SELECT pgc_dump.format_type('%u'::pg_catalog.oid, NULL)",
					  oid);
